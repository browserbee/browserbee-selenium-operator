...
// Package main is the entry point for the Selenium Operator.
package main
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "f6c0f30d.browserbee.io",
	...
	if err = (&seleniumgridcontroller.SeleniumGridReconciler{
		...
		setupLog.Error(err, "unable to create controller", "controller", "SeleniumGrid")
		os.Exit(1)
	}
	if err = (&seleniumhubcontroller.SeleniumHubReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "SeleniumHub")
		os.Exit(1)
	}
	if err = (&seleniumnodecontroller.SeleniumNodeReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "SeleniumNode")
		os.Exit(1)
	}
	if err = (&seleniumstandalonecontroller.SeleniumStandaloneReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
